...
def load_and_reduce_gene_features(
    ...
    if cl_features.meta_info is None:
        print("Fehler: Keine Meta-Informationen in den Daten verfügbar.")
        return None
    ...
    if missing_genes:
        ...
        if len(missing_genes_list) > 10:
            print(
                f"Die folgenden Gene fehlen im Datensatz {dataset_name} für {feature_type}: "
                f"{', '.join(missing_genes_list[:10])}, ... ({len(missing_genes)} Gene insgesamt)"
            )
        else:
            print(
                f"Die folgenden Gene fehlen im Datensatz {dataset_name} für {feature_type}: "
