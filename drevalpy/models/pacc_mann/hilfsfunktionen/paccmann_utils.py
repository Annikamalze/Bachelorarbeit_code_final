...
class PaccMannV2(nn.Module):
    ...
    def forward(self, smiles, gep, confidence=False):
        ...
        if isinstance(gep, np.ndarray):
            gep = torch.tensor(gep, dtype=torch.float32)  

        if isinstance(smiles, np.ndarray):
            smiles = torch.tensor(smiles, dtype=torch.float32) 
        ...
        #print(f"smiles dtype: {smiles.dtype}")
        #print(f"Maximaler Wert in smiles: {smiles.max()}")
        #print(f"Minimaler Wert in smiles: {smiles.min()}")
        #print(f"Erwartete Vokabulargröße: {self.smiles_embedding.num_embeddings}") 
